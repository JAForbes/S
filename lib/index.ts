...
export let dependents = new WeakMap<
  StreamInternal<unknown>,
  Set<ComputationInternal<unknown>>
>();
...
export let parents = new WeakMap<
  ComputationInternal<unknown>,
  Set<ComputationInternal<unknown>>
>();
export let children = new WeakMap<
  ComputationInternal<unknown>,
  Set<ComputationInternal<unknown>>
>();
...
export type AnyMap<K, V> = K extends object
  ? WeakMap<K, V> | Map<K, V>
  : Map<K, V>;
...
export function data<T>(
  value?: T,
  equality: EqualityCheck<T> = strictEquality
) {
...
export function computation<T>(
  fn: SyncComputationVisitor<T>,
  seed: T
): Computation<T>;
export function computation<T>(
  fn: SyncComputationVisitor<T>,
  seed?: T
): Computation<T | undefined>;
export function computation<T>(
  fn: SyncComputationVisitor<T>,
  seed: T
): Computation<T> {
...
export function cleanup(f: VoidFunction): void {
  if (active[0]) {
    xet(cleanups, active[0], () => new Set()).add(f);
  } else {
