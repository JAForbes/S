...
test('on', t => {
    S.root( () => {
        ...
        let i = 0;
        let singleDep = S.on(a, () => {
            i++
            b()
            return a() + 1
        }, 0)

        t.equals(singleDep(), 1, 'first tick')
        b(1)
        t.equals(singleDep(), 1, 'no change after writing to b')
        a(1)
        t.equals(singleDep(), 2, 'change after writing to a')

        b(1);b(1);b(1);b(1);
        t.equals(2, i, '2 ticks expected')
    })
    ...
    S.root(() => {
        ...
        t.equals(singleDep(), 1, 'first tick')
        c(1)
        t.equals(singleDep(), 1, 'no change after writing to c')
        a(1)
        t.equals(singleDep(), 2, 'change after writing to a')

        b(1)
        t.equals(singleDep(), 3, 'change after writing to b')

        c(1);c(1);c(1);c(1);
        t.equals(3, i, '3 ticks expected')
    })
...
test('on + dispose', t => {
    S.root( dispose => {
        ...
        t.equals(i, 1, '1 tick expected on init')

        b(x => x + 1)

        t.equals(i, 1, 'no ticks expected when updating unrelated data')

        a(x => x + 1 )

        t.equals(i, 2, 'tick expected when updated dep')

        dispose()

        a(2)

        t.equals(i, 2, 'no tick expected when updated dep but computation disposed')
    })
    t.end()
})
...
test('reducing computations', t => {
    ...
    S.root(() => {
        let example = S.computation( x => x + b(), 0 )

        t.equals(example(), 0, 'initial')
        b(1)
        t.equals(example(), 1, 'first value')
        b(2)
        t.equals(example(), 3, 'second value')
        b(3)
        t.equals(example(), 6, 'third value')
    })
    t.end()
})

test('reducing async computations', async t => {

    let b = S.data(0)
    let example = 
        S.root( () => {
            return S.generator( function * (prev) {
